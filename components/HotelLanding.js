...
export default function HotelLanding({ hotel }) {
    ...
    return (
        <Layout navbarProps={{ logo: hotel.logoUrl, brandName: hotel.heroTitle ? hotel.name : 'BookingKub' }}>
                ...
                <div className="container mx-auto px-4">
                    <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                        <div className="space-y-2">
                            <div className="text-theme-accent font-bold tracking-[0.2em] uppercase text-xs animate-fade-in-up">Accommodations</div>
                            <h2 className="text-4xl md:text-5xl font-display font-bold text-theme-text leading-tight">{t('landing.findSanctuary')}</h2>
                        </div>
                        <Link href={`/search?${defaultParams}`} className="group flex items-center gap-2 text-theme-muted hover:text-theme-accent font-medium transition-colors pb-2">
            ...
            {/* Amenities Section */}
            <section className="py-32 bg-theme-card relative overflow-hidden">
                <div className="container mx-auto px-4 relative z-10">
                    <div className="flex flex-col lg:flex-row gap-16 items-center">
                        {/* Text Content */}
                        <div className="flex-1 order-2 lg:order-1 space-y-8 animate-fade-in-left">
                            <div className="space-y-4">
                                <span className="text-theme-accent font-bold tracking-[0.2em] uppercase text-xs">{t('landing.ourAmenities')}</span>
                                <h2 className="text-4xl md:text-5xl font-display font-bold text-theme-text leading-tight">
                                    {t('landing.comfortTitle')} <br /><span className="text-theme-muted">{t('landing.comfortSubtitle')}</span>
                                </h2>
                                <p className="text-theme-muted text-lg leading-relaxed max-w-xl">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {[
                                    ...
                                    <div key={i} className="flex gap-4 p-4 rounded-2xl hover:bg-theme-bg transition-colors border border-transparent hover:border-theme-border">
                                        <div className="w-12 h-12 bg-theme-accent/10 rounded-full flex items-center justify-center text-theme-accent shrink-0">
                                            <item.icon size={22} />
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-theme-text">{item.title}</h4>
                                            <p className="text-sm text-theme-muted mt-1">{item.desc}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="flex-1 order-1 lg:order-2 w-full">
                            <div className="relative">
                                ...
                                <div className="absolute -top-10 -right-10 w-40 h-40 bg-theme-accent/10 rounded-full blur-3xl -z-10" />
                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full border border-theme-border rounded-[3rem] -z-10 rotate-3" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {/* Call to Action */}
            <section className="py-24 relative overflow-hidden bg-theme-text">
                <div className="absolute inset-0">
                    <img src="https://images.unsplash.com/photo-1571896349842-6e5c48dc52e3?q=80&w=2000" className="w-full h-full object-cover opacity-15" alt="CTA BG" />
                    <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, color-mix(in srgb, var(--theme-accent) 70%, black) 0%, rgba(10,10,20,0.85) 100%)' }} />
                </div>
                ...
                <div className="container mx-auto px-4 text-center relative z-10">
                    <motion.div
                        ...
                        viewport={{ once: true }}
                    >
                        <h2 className="text-4xl md:text-6xl font-display font-bold mb-8 text-white">{t('landing.readyStay')}</h2>
                        <p className="text-white/70 text-xl max-w-2xl mx-auto mb-10">
                            {t('landing.bookDirectly')}
                        </p>
                        <Link href={`/search?${defaultParams}`} className="btn-theme inline-flex items-center gap-3 text-lg px-10 py-5 rounded-full shadow-2xl hover:scale-105 transition-all duration-300">
