...
const HK_CONFIG = {
    CLEAN:     { label: 'Clean',        color: 'bg-sky-500 text-white', icon: <Sparkles size={10} /> },
    INSPECTED: { label: 'Ready',        color: 'bg-emerald-500 text-white', icon: <CheckCircle size={10} /> },
    DIRTY:     { label: 'Dirty',        color: 'bg-red-500 text-white', icon: <AlertCircle size={10} /> },
    CLEANING:  { label: 'Cleaning',     color: 'bg-amber-500 text-white', icon: <Clock size={10} /> },
    OOO:       { label: 'Out of Order', color: 'bg-slate-600 text-white', icon: <Wrench size={10} /> },
}
...
export default function Calendar() {
    ...
    const COL_MIN_W = viewMode === 'day' ? 200 : viewMode === '3day' ? 160 : viewMode === 'week' ? 100 : viewMode === '2week' ? 56 : 36
    ...
    const availablePerDay = useMemo(() => {
        ...
        days.forEach(day => {
            const s = new Date(day); s.setHours(0, 0, 0, 0)
            const e = new Date(day); e.setHours(23, 59, 59, 999)
            const booked = new Set(events
                .filter(ev => filterConfig.status.includes(ev.status) && new Date(ev.checkIn) < e && new Date(ev.checkOut) > s)
                .map(ev => ev.roomId))
            map[day.toDateString()] = Math.max(0, rooms.length - booked.size)
        })
        return map
    }, [days, events, rooms, filterConfig.status])
    ...
    const handleDrop = async (room, date) => {
        ...
        setConfirmModal({
            ...
            message: `Move ${draggedBooking.leadName} to Room ${room.roomNumber || room.id.slice(-4)} (${newCheckIn.toLocaleDateString('en-GB')} to ${newCheckOut.toLocaleDateString('en-GB')})?`,
            type: 'warning',
            onConfirm: async () => {
                const hotelId = currentHotel?.id;
                try {
    ...
    const hasFilter = filterConfig.roomTypeId !== 'All' || filterConfig.status.length < 3
            ...
            <div className="flex flex-col h-[calc(100vh-64px)] gap-2 relative">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex items-center bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden shadow-sm">
                        <button onClick={() => {
                            ...
                            const delta = { day: 1, '3day': 3, week: 7, '2week': 14, month: 0 }[viewMode]
                            delta ? d.setDate(d.getDate() - delta) : d.setMonth(d.getMonth() - 1)
                            setCurrentDate(d)
                    ...
                    <div className="hidden md:flex items-center gap-1.5 text-[10px] font-medium border-l border-slate-200 dark:border-slate-700 pl-2 ml-0.5">
                        {[
                            { label: 'In House',  value: stats.inHouse,    dot: 'bg-emerald-500' },
                            ...
                            <table className="w-full text-xs">
                                ...
                                <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
                                    {loading ? (
                                        ...
                                        rooms.filter(r => r.roomTypeId === type.id).map(room => {
                                            ...
                                            return (
                                                <tr key={room.id} className={`transition-colors ${isFree ? 'hover:bg-emerald-50/40 dark:hover:bg-emerald-900/10' : 'hover:bg-slate-50 dark:hover:bg-slate-700/20'}`}>
                                                    <td className="px-3 py-2.5 font-bold text-slate-800 dark:text-white">{room.roomNumber || room.id.slice(-4)}</td>
                                                    <td className="px-3 py-2.5 text-slate-500">{type.name}</td>
                                                    <td className="px-3 py-2.5">
                                                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${hk.color}`}>
                                                    ...
                                                    <td className="px-3 py-2.5 text-slate-500">{active ? `${nights}N` : '-'}</td>
                                                    <td className="px-3 py-2.5">
                                                        {cfg ? (
                                                            <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded text-white ${cfg.bar.split(' ')[0]}`}>
                                                                {cfg.label}
                                                            </span>
                                                        ) : <span className="text-emerald-500 font-bold text-[9px]">FREE</span>}
                                                    </td>
                                                    <td className="px-3 py-2.5 text-right">
                                                        {active ? (
                                                            ...
                                                            <button onClick={() => handleCellClick(room, today)} className="text-[9px] font-bold px-2 py-1 bg-emerald-500 hover:bg-emerald-600 text-white rounded transition-colors">
                                                                + Book
                                                            </button>
                                                        )}
                                                    </td>
                        ...
                        <div style={{ minWidth: `${ROOM_COL + days.length * COL_MIN_W}px` }}>
                            ...
                            <div className="flex sticky top-0 z-20 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shadow-sm">
                                ...
                                {days.map(day => {
                                    ...
                                    const avail = availablePerDay[day.toDateString()] ?? 0
                                    return (
                                        <div
                                            key={day.toISOString()}
                                            className={`flex-1 py-1 text-center border-r border-slate-100 dark:border-slate-700/60 ${
                                                today ? 'bg-blue-50 dark:bg-blue-900/20' : isWeekend(day) ? 'bg-slate-50 dark:bg-slate-900/50' : ''
                                            }`}
                                            style={{ minWidth: COL_MIN_W }}
                                        >
                                            <div className={`text-[9px] uppercase font-bold ${today ? 'text-blue-500' : 'text-slate-400'}`}>
                                                {viewMode === 'week'
                                            ...
                                            <div className={`text-[11px] font-bold mx-auto leading-none mt-0.5 ${
                                                today
                                                    ? 'w-5 h-5 bg-blue-500 text-white rounded-full flex items-center justify-center mx-auto'
                                                    : isWeekend(day) ? 'text-slate-500 dark:text-slate-400' : 'text-slate-700 dark:text-slate-200'
                                            }`}>
                                                {day.getDate()}
                                            </div>
                                            <div className={`text-[8px] font-bold mt-0.5 leading-none ${
                                                avail === 0 ? 'text-red-400' : avail <= 5 ? 'text-amber-500' : 'text-emerald-500 dark:text-emerald-400'
                                            }`}>
                                                {avail === 0 ? '●' : avail}
                                            </div>
                                        </div>
                            ...
                            {loading ? (
                                ...
                                const visibleRooms = showAvailableOnly ? typeRooms.filter(r => roomHasAnyFreeDay(r.id)) : typeRooms

                                return (
                                    <div key={type.id}>
                                        {/* Room type group header */}
                                        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                            <Layers size={11} />
                                            {type.name}
                                            <span className="ml-1 font-medium normal-case text-slate-400 bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 rounded-full">
                                                {showAvailableOnly ? `${visibleRooms.length}/` : ''}{typeRooms.length}
                                            </span>
                                        </div>
                                        ...
                                        {visibleRooms.map(room => {
                                            ...
                                            return (
                                                <div key={room.id} className="flex border-b border-slate-100 dark:border-slate-700/50 h-12 relative hover:bg-slate-50/60 dark:hover:bg-slate-700/10 transition-colors">
                                                    ...
                                                    >
                                                        ...
                                                        <button
                                                            ...
                                                            className={`text-[8px] font-bold px-1 py-0.5 rounded flex-shrink-0 flex items-center gap-0.5 ${hk.color}`}
                                                        >
                                                            <span className="flex items-center justify-center opacity-90">{hk.icon}</span> {room.status === 'INSPECTED' ? 'RDY' : (room.status || 'CLN').slice(0, 3)}
                                                        </button>
                                                    </div>
                                                    ...
                                                    {days.map(day => {
                                                        ...
                                                        return (
                                                            <div
                                                                ...
                                                                title={free ? 'ว่าง — คลิกเพื่อจอง' : 'มีการจองแล้ว'}
                                                                style={{ minWidth: COL_MIN_W }}
                                                                className={`flex-1 border-r border-slate-50 dark:border-slate-700/30 h-full cursor-pointer transition-colors relative group overflow-hidden ${
                                                                    isToday(day)
                                                                        ? free ? 'bg-blue-50/40 dark:bg-blue-900/10' : 'bg-blue-100/30 dark:bg-blue-900/20'
                                                                        : free
                                                                        ? 'bg-emerald-50/20 dark:bg-emerald-900/5 hover:bg-emerald-100/60'
                                                            ...
                                                            >
                                                                {free && <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 text-emerald-600/50 font-bold text-[10px] bg-emerald-100/40 backdrop-blur-[1px]">+ Book</div>}
                                                            </div>
                                                        )
                                                    })}
                    ...
                    >
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center gap-1.5"><Clock size={10} className="text-slate-400"/>
                                {new Date(hoveredBooking.checkIn).toLocaleDateString('th-TH')} → {new Date(hoveredBooking.checkOut).toLocaleDateString('th-TH')}
                            </div>
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className={`text-[9px] px-1 py-0.5 rounded font-bold ${bookingPaymentStatus(hoveredBooking) === 'PAID' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                ...
                {hkTooltip.isOpen && hkTooltip.room && (
                    ...
                    >
                        ...
                        ) : <p className="text-slate-400 italic text-[10px]">No recent logs</p>}
                    </div>
                )}
            </div>
