...
export default function InvoicePage() {
    ...
    const { booking, lineItems, totalInvoiceAmount, hotel } = invoiceData;

    return (
        <div className="min-h-screen bg-slate-100 print:bg-white pb-20">
            <Head>
                <title>Invoice #{booking.id.substring(booking.id.length - 6).toUpperCase()}</title>
            </Head>
            ...
            <div className="max-w-4xl mx-auto bg-white shadow-xl print:shadow-none print:max-w-full overflow-hidden sm:rounded-2xl">
                    ...
                    <div className=" md:text-right">
                        ...
                        <div className="flex flex-col gap-1 text-sm md:items-end">
                            <div className="flex justify-between md:justify-end gap-8">
                                ...
                                <span className="font-bold text-slate-900 border-b border-dashed border-slate-300">#{booking.id.substring(booking.id.length - 8).toUpperCase()}</span>
                            </div>
                            <div className="flex justify-between md:justify-end gap-8">
                ...
                <div className="p-8 md:p-12 grid grid-cols-1 md:grid-cols-2 gap-12 border-b border-slate-100 bg-slate-50/50">
                    <div>
                        ...
                        <p className="text-lg font-bold text-slate-900 mb-1">{booking.leadName}</p>
                        <p className="text-sm text-slate-600 mb-1">{booking.leadEmail}</p>
                        <p className="text-sm text-slate-600">{booking.leadPhone}</p>
                    </div>
                    <div className="md:text-right">
                        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Stay Details</h3>
                        <div className="flex flex-col gap-2 md:items-end">
                            <p className="text-sm"><span className="text-slate-500">Check-in:</span> <span className="font-bold">{new Date(booking.checkIn).toLocaleDateString('en-GB')}</span></p>
                            <p className="text-sm"><span className="text-slate-500">Check-out:</span> <span className="font-bold">{new Date(booking.checkOut).toLocaleDateString('en-GB')}</span></p>
                            <p className="text-sm"><span className="text-slate-500">Guests:</span> <span className="font-bold">{booking.guestsAdult} Adults, {booking.guestsChild} Children</span></p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="mt-8 flex justify-end">
                        <div className="w-full md:w-1/2 rounded-2xl bg-slate-50 p-6 border border-slate-100">
                            <div className="flex justify-between items-center mb-3 text-slate-600">
                                ...
                                <span className="font-bold">฿{totalInvoiceAmount.toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between items-center mb-4 text-slate-600">
                                <span className="text-sm font-medium">Tax & Fees (Included)</span>
                            ...
                            <div className="pt-4 border-t-2 border-slate-200 flex justify-between items-center">
                                ...
                                <span className="text-3xl font-black text-indigo-600 tracking-tight">฿{totalInvoiceAmount.toLocaleString()}</span>
                            </div>
                        </div>
                    </div>
                </div>
